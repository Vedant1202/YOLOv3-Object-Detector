...
def print_outputs(img, boxes, objectness, classes, nums, class_names, img_name):
    boxes, objectness, classes, nums = boxes[0], objectness[0], classes[0], nums[0]
    boxes = np.array(boxes)
    for i in range(nums):
        x1y1 = tuple((boxes[i, 0:2] * [img.shape[1], img.shape[0]]).astype(np.int32))
        ...
        # Display only those boxes having class 'dog'
        if class_names[int(classes[i])] == 'dog':
                    ...
                    # print(float(objectness[i]))
                    img = cv2.imread(img_name)
                    crop_img = img[x1y1[1]:x2y2[1], x1y1[0]:x2y2[0]]
                    cv2.imshow(class_names[int(classes[i])], crop_img)
                    cv2.waitKey(0)
